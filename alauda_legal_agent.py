...
# ---------------------------------------------------------
def run_llm_inference(text: str, mode: str, api_key: str, model_provider: str = "google", base_url: Optional[str] = None):
    ...
    print(f"⏳ 正在调用 {model_provider} 大模型引擎进行深度 {'单文档语义分析' if mode == 'single' else '多文档图谱交叉审查'}...")
    try:
        return chain.invoke({"contract_text": text[:500000], "format_instructions": parser.get_format_instructions()})
    except Exception as e:
        error_msg = str(e)
        print(f"❌ API 调用或解析失败: {error_msg}")
