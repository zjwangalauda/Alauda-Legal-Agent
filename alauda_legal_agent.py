...
class COODecision(BaseModel):
    ...
    strategic_advice: str = Field(description="写给COO的战略建议（结合法务风险与商务收益的综合博弈分析，如：虽然存在20%违约金风险，但鉴于是头部标杆金融客户且首付比例高，建议通过提高交付质量来对冲法务风险）")
...
class ComprehensiveReviewReport(BaseModel):
    ...
    # 3. 高管决策视角
    coo_view: COODecision = Field(description="给公司COO/高管的最终签约审批建议")
...
class MultiDocReviewReport(BaseModel):
    ...
    # 3. 高管决策视角
    coo_view: COODecision = Field(description="给公司COO/高管的跨文档综合签约审批建议")
...
3. 【COO 决策视角】：结合商务收益和法务风险，权衡利弊，向公司 COO 给出极其明确的最终签字审批建议。
...
4. 【COO 决策视角】：结合整套案卷的商业利益与暗藏风险，向公司 COO 给出综合且明确的审批决策建议。
...
# ---------------------------------------------------------
def run_llm_inference(text: str, mode: str, api_key: str, model_provider: str = "google", base_url: str = None):
    ...
    if model_provider == "google":
        ...
        llm = ChatOpenAI(model="gpt-4o", temperature=0.1, max_tokens=8192, api_key=api_key, base_url=base_url)
...
# ---------------------------------------------------------
def render_single_doc_report(report: ComprehensiveReviewReport, output_file: str):
    md = f"# 🚀 Alauda AI Agent (V5) 综合决议报告 - 单文档\n\n"
    md += f"> **案卷名称**: {report.contract_name}\n"
    md += f"> **智能引擎**: Gemini Pro (Legal & Commercial Copilot)\n\n"
    
    md += f"## 👨‍💼 COO 最终审批台 (Executive Decision)\n"
    md += f"- **审批建议**: **{report.coo_view.approval_recommendation}**\n"
    md += f"- **致命阻碍 (Deal Breakers)**: {report.coo_view.deal_breaker_summary}\n"
    md += f"- **战略博弈指导**: {report.coo_view.strategic_advice}\n\n"
    md += "---\n"

    md += f"## 📈 运营与商务提炼 (Commercial Operations View)\n"
    ...
    if not report.legal_reviews: md += "🎉 **合规通关！** 未检测到突破 SaaS 商业红线的高危条款。\n\n"
